...
export default class AppHeader extends Component {
    ...
    constructor(props, ...args) {
        ...
        this.$$container.className = 'app-header abs box row tc';
    ...
    render() {
        ...
        return show ? createPortal(
            [
                <div key="left" className={ 'app-nav-left flex rel row' + (navBack ? ' has-nav-back' : '') }>
                    { this.renderNavBack() }
                    <AppTransition className="row pl10 ml5" component={ navLeft || 'left' } />
                </div>,
                <h1 key="title" className="app-title flex rel f18">
                    <AppTransition className="nowrap" type={ type } component={ title } />
                </h1>,
                <div key="right" className="app-nav-right flex rel">
